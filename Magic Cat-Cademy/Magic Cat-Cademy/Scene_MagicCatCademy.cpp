...
void Scene_MagicCatCademy::sDoAction(const Command& action)
{
	...
	if (action.type() == "START") {
		...
		if (!bossDefeated) {
			...
			else if (action.name() == "SHOOT" && !m_isPaused) {
...
void Scene_MagicCatCademy::sUpdate(sf::Time dt)
{
	...
	auto pauseUiResume = m_entityManager.getEntities("pauseUiResume");
	auto pauseUiQuit = m_entityManager.getEntities("pauseUiQuit");
	...
	if (m_isPaused) {
		...
		for (auto p : pauseUiResume) {
			p->getComponent<CTransform>() = m_worldView.getCenter();
	...
	for (auto p : pauseUiQuit) {
		p->getComponent<CTransform>() = sf::Vector2f(m_worldView.getCenter().x, m_worldView.getCenter().y + 200);
		auto& sprite = p->getComponent<CAnimation>().animation.getSprite();
		auto color = sprite.getColor();
		color.a = 255;
		sprite.setColor(color);
	...
	void Scene_MagicCatCademy::sCollision(sf::Time dt)
	{
		...
		auto quitUi = m_entityManager.getEntities("pauseUiQuit");
		auto resumeUi = m_entityManager.getEntities("pauseUiResume");
		...
		for (auto m : magic) {
			for (auto h : hellhounds) {
				...
				if (overlap.x > 0 && overlap.y > 0) {
					...
					m->destroy();
					h->getComponent<CHealth>().hp -= m_config.magicStrength;
					checkIfDead(h);
			...
			for (auto mars : m_entityManager.getEntities("mars")) {
				...
				if (overlap.x > 0 && overlap.y > 0) {
					...
					m->destroy();
					mars->getComponent<CHealth>().hp -= m_config.magicStrength;
					checkIfDead(mars);
				}
			}
		}
	...
	// Functions for spawning of entities
	...
	// Functions for things to be rendered
	...
	void Scene_MagicCatCademy::drawPauseUI()
	{
		sf::Vector2f resumePos = sf::Vector2f(m_game->window().getSize().x / 2.0f, m_game->window().getSize().y / 2.0f);
		sf::Vector2f quitPos = sf::Vector2f(m_game->window().getSize().x / 2.0f, m_game->window().getSize().y + 100 / 2.0f);

		// Add a new entity for the pause UI
		std::cout << "resume pos " << resumePos << "\n";
		std::cout << "quit pos " << quitPos << "\n";
		auto resume = m_entityManager.addEntity("pauseUiResume");
		resume->addComponent<CTransform>(resumePos);
		resume->addComponent<CAnimation>(Assets::getInstance().getAnimation("pauseUiResume"));
		resume->addComponent<CBoundingBox>(sf::Vector2f(500, 100.f));
		resume->getComponent<CAnimation>().animation.getSprite().setColor(sf::Color(255, 255, 255, 0));

		auto quit = m_entityManager.addEntity("pauseUiQuit");
		quit->addComponent<CTransform>(quitPos);
		quit->addComponent<CBoundingBox>(sf::Vector2f(200, 100.f));
		quit->addComponent<CAnimation>(Assets::getInstance().getAnimation("pauseUiQuit"));
		quit->getComponent<CAnimation>().animation.getSprite().setColor(sf::Color(255, 255, 255, 0));
	}
		...
		if (e->hasComponent<CHealth>()) {
			if (e->getComponent<CHealth>().hp <= 0) {
				...
				if (e->getTag() == "lucy") {
					...
					for (auto h : m_entityManager.getEntities("hellhound")) h->destroy();
					for (auto p : m_entityManager.getEntities("powerup")) p->destroy();
					for (auto m : m_entityManager.getEntities("mars")) m->destroy();
					for (auto s : m_entityManager.getEntities("sierra")) s->destroy();
					for (auto c : m_entityManager.getEntities("cage")) c->destroy();

					if (m_config.lives == 0) onEnd();
